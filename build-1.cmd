...
rem In theory, the following should work, but doesn't:
rem factor.exe -e="USE: bootstrap.image \"windows-x86.32\" make-image" -run=none

echo Build phase 1 complete.
echo Now run factor.exe and execute
echo USE: bootstrap.image "windows-x86.32" make-image
echo to produce a new bootstrap image.
echo Then run build-2.cmd to complete the build.
goto :EOF
